(function() {
  ...
  var wsUrl = null,
    ...
    CELL_POINTS_MIN = 3, // Default = 0
    CELL_POINTS_MAX = 3, // Default = 0
    VIRUS_POINTS = 3,
    PI_2 = Math.PI * 2,
  ...
  function drawStats() {
    ...
    var rows = [
      ...
      (stats.info.update * 2.5).toFixed(1) +
        "% load @ " +
  ...
  function drawGame() {
    ...
    if (frozen) {
      ...
      mainCtx.textBaseline = "hanging";
      mainCtx.fillStyle = "#eea236";
      var text = "You are frozen, press F to unfreeze.";
      mainCtx.fillText(text, mainCanvas.width / 2, 5);
      mainCtx.restore();
    }
  ...
  function keydown(event) {
    ...
    if (key == "enter") {
      ...
      /*                              if (key == "e") macroIntervalID = setInterval(function() {
                wsSend(code);
            }, macroCooldown);*/

      if (key == "q") minionControlled = !minionControlled;
      if (key == "f") frozen = !frozen;
    }
    if (event.keyCode == 90) {
      // Z-key (x16 split)
      ...
      /*              setTimeout(function() {wsSend(UINT8_CACHE[17])}, 200)
                setTimeout(function() {wsSend(UINT8_CACHE[17])}, 225)
                setTimeout(function() {wsSend(UINT8_CACHE[17])}, 250)
                setTimeout(function() {wsSend(UINT8_CACHE[17])}, 275)
                setTimeout(function() {wsSend(UINT8_CACHE[17])}, 300)
                setTimeout(function() {wsSend(UINT8_CACHE[17])}, 325) */
    }
    if (event.keyCode == 83) {
      // S-key (triple split)
  ...
  function keyup(event) {
    ...
    if (key == "f") wsSend(UINT8_CACHE[70]);
    else if (key == "w") clearInterval(macroIntervalID);
    else if (key == "r") clearInterval(macroIntervalID);
    //           else if (key == "e") clearInterval(macroIntervalID);
  }
